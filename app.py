# ============================
# app.py — GrowthOracle AI — Next Gen (FIXED)
...
if step == "2) Upload & Map Columns":
    ...
    with st.expander("Production Mapping", expanded=True):
        ...
        prod_map = {}
        prod_map["msid"]   = c1.selectbox("MSID", prod_df_raw.columns, index=max(0, list(prod_df_raw.columns).index(prod_map_guess.get("msid","")) if prod_map_guess.get("msid") in prod_df_raw.columns else 0))
        prod_map["title"]  = c2.selectbox("Title", prod_df_raw.columns, index=max(0, list(prod_df_raw.columns).index(prod_map_guess.get("title","")) if prod_map_guess.get("title") in prod_df_raw.columns else 0))
        prod_map["path"]   = c3.selectbox("Path", prod_df_raw.columns, index=max(0, list(prod_df_raw.columns).index(prod_map_guess.get("path","")) if prod_map_guess.get("path") in prod_df_raw.columns else 0))
        prod_map["publish"]= c4.selectbox("Publish Time", prod_df_raw.columns, index=max(0, list(prod_df_raw.columns).index(prod_map_guess.get("publish","")) if prod_map_guess.get("publish") in prod_df_raw.columns else 0))

    with st.expander("GA4 Mapping", expanded=True):
        c1, c2, c3, c4, c5 = st.columns(5)
        ga4_map = {}
        ga4_map["msid"]      = c1.selectbox("MSID", ga4_df_raw.columns, index=max(0, list(ga4_df_raw.columns).index(ga4_map_guess.get("msid","")) if ga4_map_guess.get("msid") in ga4_df_raw.columns else 0))
        ga4_map["date"]      = c2.selectbox("Date", ga4_df_raw.columns, index=max(0, list(ga4_df_raw.columns).index(ga4_map_guess.get("date","")) if ga4_map_guess.get("date") in ga4_df_raw.columns else 0))
        ga4_map["pageviews"] = c3.selectbox("Pageviews", ga4_df_raw.columns, index=max(0, list(ga4_df_raw.columns).index(ga4_map_guess.get("pageviews","")) if ga4_map_guess.get("pageviews") in ga4_df_raw.columns else 0))
        ga4_map["users"]     = c4.selectbox("Users", ga4_df_raw.columns, index=max(0, list(ga4_df_raw.columns).index(ga4_map_guess.get("users","")) if ga4_map_guess.get("users") in ga4_df_raw.columns else 0))
        ga4_map["engagement"]= c5.selectbox("Engagement Duration", ga4_df_raw.columns, index=max(0, list(ga4_df_raw.columns).index(ga4_map_guess.get("engagement","")) if ga4_map_guess.get("engagement") in ga4_df_raw.columns else 0))
        ga4_map["bounce"]    = st.selectbox("Bounce Rate", ga4_df_raw.columns, index=max(0, list(ga4_df_raw.columns).index(ga4_map_guess.get("bounce","")) if ga4_map_guess.get("bounce") in ga4_df_raw.columns else 0))

    with st.expander("GSC Mapping", expanded=True):
        c1, c2, c3, c4, c5, c6, c7 = st.columns(7)
        gsc_map = {}
        gsc_map["date"] = c1.selectbox("Date", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("date","")) if gsc_map_guess.get("date") in gsc_df_raw.columns else 0))
        gsc_map["page"] = c2.selectbox("Page URL", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("page","")) if gsc_map_guess.get("page") in gsc_df_raw.columns else 0))
        gsc_map["query"]= c3.selectbox("Query", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("query","")) if gsc_map_guess.get("query") in gsc_df_raw.columns else 0))
        gsc_map["clicks"]= c4.selectbox("Clicks", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("clicks","")) if gsc_map_guess.get("clicks") in gsc_df_raw.columns else 0))
        gsc_map["impr"] = c5.selectbox("Impressions", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("impr","")) if gsc_map_guess.get("impr") in gsc_df_raw.columns else 0))
        gsc_map["ctr"]  = c6.selectbox("CTR", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("ctr","")) if gsc_map_guess.get("ctr") in gsc_df_raw.columns else 0))
        gsc_map["pos"]  = c7.selectbox("Position", gsc_df_raw.columns, index=max(0, list(gsc_df_raw.columns).index(gsc_map_guess.get("pos","")) if gsc_map_guess.get("pos") in gsc_df_raw.columns else 0))
...
def process_uploaded_files(prod_df_raw, ga4_df_raw, gsc_df_raw, prod_map, ga4_map, gsc_map,
    ...
    prod_df = prod_df_raw.copy()
    ga4_df = ga4_df_raw.copy()
    gsc_df = gsc_df_raw.copy()
    ...
    # Rename GSC columns (FIXED: Added clicks handling)
    gsc_ren = {
        gsc_map["date"]: "date", 
    ...
    # Add Clicks if it exists
    if gsc_map.get("clicks") and gsc_map["clicks"] in gsc_df.columns:
        gsc_ren[gsc_map["clicks"]] = "Clicks"
    ...
    # Handle Clicks if present
    if "Clicks" in gsc_df.columns:
        gsc_df["Clicks"] = coerce_numeric(gsc_df["Clicks"], "GSC.Clicks", vc, clamp=(0, float("inf")))
    ...
    if "CTR" in gsc_df.columns:
        ...
        if out: 
            vc.add("Warning","CTR_CLAMP","CTR values clamped to [0,1]", rows=int(out))
    ...
    for tag, df in [("GA4", ga4_df), ("GSC", gsc_df)]:
        if "date" in df.columns:
            ...
            if dd.notna().any(): 
                d_range[tag]=(dd.min(), dd.max())
    
    if len(d_range)==2:
        ga4_min, ga4_max = d_range["GA4"]
        gsc_min, gsc_max = d_range["GSC"]
        latest_start = max(ga4_min, gsc_min)
        earliest_end = min(ga4_max, gsc_max)
        overlap_days = (earliest_end - latest_start).days if earliest_end>=latest_start else -1
        
        if overlap_days < 0: 
            vc.add("Warning","DATE_NO_OVERLAP","GA4 and GSC dates do not overlap", 
                   ga4=str(d_range["GA4"]), gsc=str(d_range["GSC"]))
        elif overlap_days < 7: 
            vc.add("Info","DATE_SMALL_OVERLAP",f"Small GA4↔GSC overlap (~{overlap_days} days)", 
    ...
    if len(subset_cols)==3:
        ...
        if dup_before: 
            vc.add("Info","DEDUP","Duplicate rows removed on (date, msid, Query)", count=int(dup_before))
...
# Run processing
vc_serialized = rep_df.to_json(orient="records") if not rep_df.empty else None

with st.spinner("Processing & merging with robust validation..."):
    master_df, vc_after = process_uploaded_files(prod_df_raw, ga4_df_raw, gsc_df_raw, 
...
if not post_df.empty:
    ...
    st.download_button("Download Post-merge Report (CSV)", 
...
if "date" in master_df.columns:
    try:
        ...
        if pd.notna(fresh): 
            st.caption(f"Latest date in data: **{pd.to_datetime(fresh).date()}**")
    except Exception:
        pass
...
if master_df.shape[0] > CONFIG["performance"]["sample_row_limit"]:
    ...
    master_df = master_df.sample(CONFIG["performance"]["sample_row_limit"], 
...
if step == "3) Validate & Process":
    ...
    st.stop()
    # ============================
# PART 4/5: Filters, Helpers, Core Insight Modules & Charts (FIXED)
# ============================
...
def export_plot_html(fig, name):
    if to_html is None:
        st.info("Plotly HTML export not available in this environment.")
        return
    html_str = to_html(fig, include_plotlyjs="cdn", full_html=True)
    st.download_button("Export Chart (HTML)", 
                       data=html_str.encode("utf-8"),
                       file_name=f"{name}.html", 
                       mime="text/html")

# ============================
# Module 4: Engagement vs Search Mismatch (ROBUST & FIXED)
# ============================

def _pick_col(d, candidates):
    """Pick first available column from candidates"""
    return next((c for c in candidates if c in d.columns), None)

def _build_clicks_proxy(d):
    """Create click-like signal from available metrics"""
    if "Impressions" in d.columns and "CTR" in d.columns:
        ...
        return (imp * ctr).fillna(0)
    
    if "screenPageViews" in d.columns:
        return pd.to_numeric(d["screenPageViews"], errors="coerce").fillna(0)
    
    if "totalUsers" in d.columns:
        return pd.to_numeric(d["totalUsers"], errors="coerce").fillna(0)
    
    return None

def engagement_mismatches(df):
    """Identify content with engagement/search performance mismatches"""
    d = df.copy()
    ...
    if pos_col and pos_col in d.columns:
        d = d[(d[pos_col].isna()) | (d[pos_col].between(1, 50, inclusive="both"))].copy()

    # Engagement score (rank-based; use whatever we have)
    parts_e = []
    if dur_col and dur_col in d.columns: 
        parts_e.append(d[dur_col].rank(pct=True))
    if br_col and br_col in d.columns:  
        parts_e.append(1 - d[br_col].rank(pct=True))
    ...
    if clicks_col and clicks_col in d.columns: 
        parts_s.append(d[clicks_col].rank(pct=True))
    if pos_col and pos_col in d.columns:    
        parts_s.append(1 - d[pos_col].rank(pct=True))  # better (lower) pos => higher score
    ...
    for _, row in mismatches.iterrows():
        ...
        if row["mismatch_type"]=="Hidden Gem":
            ...
            goal = f"_Goal: Leverage high engagement to improve search visibility{'' if not pos_col else f' from position {row.get(pos_col, 0):.1f}'}._"
        else:
            recs = [
            ...
            goal = f"_Goal: Improve user experience to match search performance._"
        ...
        with st.expander(f"Why this recommendation? — {emoji} {str(row.get(title_col, 'Untitled'))[:90]}", expanded=False):
            ...
            if dur_col and dur_col in row: 
                st.write(f"- **Duration**: {row[dur_col]:.1f}s")
            if br_col and br_col in row:  
                st.write(f"- **Bounce**: {row[br_col]:.1%}")
            if pos_col and pos_col in row: 
                st.write(f"- **Pos**: {row[pos_col]:.1f}")
            if clicks_col and clicks_col in row: 
                st.write(f"- **Clicks-like signal available**")
        ...
        cards.append(
            ...
            + (f"Duration: **{row[dur_col]:.1f}s** | " if dur_col and dur_col in row else "")
            + (f"Bounce: **{row[br_col]:.1%}** | " if br_col and br_col in row else "")
            + (f"Pos: **{row[pos_col]:.1f}**" if pos_col and pos_col in row else "")
            + ("\n\n**Recommendations:**\n" + "\n".join(recs) + f"\n\n{goal}")
        )
...
def scatter_engagement_vs_search(df):
    """Create scatter plot of engagement vs search performance"""
    ...
    parts_e = []
    if dur_col and dur_col in t.columns: 
        parts_e.append(t[dur_col].rank(pct=True))
    if br_col and br_col in t.columns:  
        parts_e.append(1 - t[br_col].rank(pct=True))
    
    if not parts_e:
        st.info("No engagement signals for scatter plot.")
        return
    
    t["engagement_score"] = np.mean(parts_e, axis=0) if len(parts_e) > 1 else parts_e[0]

    parts_s = []
    if clicks_col and clicks_col in t.columns: 
        parts_s.append(t[clicks_col].rank(pct=True))
    if pos_col and pos_col in t.columns:    
        parts_s.append(1 - t[pos_col].rank(pct=True))
    
    if not parts_s:
        st.info("No search signals for scatter plot.")
        return
    ...
    agg = t.groupby(["msid","Title","L2_Category"], as_index=False).agg(
        ...
        Position=(pos_col,"mean") if pos_col and pos_col in t.columns else ("L2_Category","size"),
        Clicks=(clicks_col,"sum") if clicks_col and clicks_col in t.columns else ("L2_Category","size")
    )

    if px is None:
        st.info("Plotly not available; showing a basic table instead.")
        st.dataframe(agg[["msid","Title","engagement_score","search_score"]].head(20), 
...
st.divider()
# ============================
# PART 5/5: Category Performance & Trends/Forecasts (FIXED)
# ============================

# Heatmap helper
def category_heatmap(df, value_col, title):
    """Generate heatmap for category performance"""
    if px is None:
        st.info("Plotly not available for heatmaps.")
        return
    ...
    fig = px.density_heatmap(
        agg, x="L1_Category", y="L2_Category", z="val", 
...
def time_series_trends(df, metric_col, title):
    ...
    if "date" not in df.columns:
        st.info("No date column for time series.")
        return
    ...
    if metric_col not in t.columns:
        st.info(f"Metric '{metric_col}' not found in data.")
        return
    ...
    st.plotly_chart(fig, use_container_width=True, theme="streamlit")
    export_plot_html(fig, f"time_series_{metric_col}")
...
if not isinstance(cat_df, pd.DataFrame) or cat_df.empty:
    ...
    if "total_traffic" in cat_df.columns and cat_df["total_traffic"].sum() > 0:
        ...
        value_col = "screenPageViews" if "screenPageViews" in filtered_df.columns else (
            "totalUsers" if "totalUsers" in filtered_df.columns else None
        )
        if value_col:
    ...
    if "total_gsc_clicks" in cat_df.columns and cat_df["total_gsc_clicks"].sum() > 0:
        ...
        top_cats = cat_df.nlargest(10, "total_gsc_clicks")
        st.bar_chart(top_cats.set_index("L2_Category")["total_gsc_clicks"])
...
with col1:
    ...
    st.metric("Date Range", f"{len(filtered_df['date'].unique()) if 'date' in filtered_df.columns else 0} days")
...
with col3:
    if "screenPageViews" in filtered_df.columns:
        st.metric("Total Pageviews", f"{filtered_df['screenPageViews'].sum():,.0f}")
    if "totalUsers" in filtered_df.columns:
        st.metric("Total Users", f"{filtered_df['totalUsers'].sum():,.0f}")
...
with col_exp1:
    download_df_button(filtered_df, f"growthoracle_export_{pd.Timestamp.now().strftime('%Y%m%d')}.csv", 
                      "Download Filtered Master Data (CSV)")

with col_exp2:
    if not cat_df.empty:
        download_df_button(cat_df, f"category_performance_{pd.Timestamp.now().strftime('%Y%m%d')}.csv",
                          "Download Category Performance (CSV)")

st.success("✅ Analysis complete! Review insights above and download exports as needed.")
